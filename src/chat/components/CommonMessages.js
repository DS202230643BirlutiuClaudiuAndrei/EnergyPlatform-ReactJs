...
function CommonMessages(props) {
  ...
  const fetchMessages = () => {
    ...
    api.request("getCommonMessages").then(
  ...
  const sendMessage = () => {
    if (sock) {
      ...
      const request = {
        jsonrpc: "2.0",
        method: "sendCommonMessage",
        params: [props.user.id, messageToSend],
        id: 1,
      };
      sock.sendMessage("/app/user/commonMessage", JSON.stringify(request));
    }
  };
  ...
  const handleTypingMessage = (msg) => {
    if (msg.result.userId !== props.user.id) {
      setTyping(msg.result.firstName + "...");
